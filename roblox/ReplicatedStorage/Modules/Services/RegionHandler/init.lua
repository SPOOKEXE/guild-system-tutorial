...
function Module.Init(_)
	...
	for _, RegionInstance in ipairs( CollectionService:GetTagged('REGION_PART') ) do
		task.defer(Module.OnInstanceAdded, RegionInstance)
	end
	CollectionService:GetInstanceAddedSignal('REGION_PART'):Connect(Module.OnInstanceAdded)
	CollectionService:GetInstanceRemovedSignal('REGION_PART'):Connect(Module.OnInstanceRemoved)
end
